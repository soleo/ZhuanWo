...
module.exports = {
        ...
        alphaID: function (input, toNum, padUp, passKey) {
            ...
            if(passKey.length > 0) {
                ...
                for (var n = 0; n < index.length; n++) {
                    i.push(index.substr(n, 1));     
                } 
                var shasum = crypto.createHash('sha256');
                shasum.update(passKey);
                var passhash = shasum.digest('hex');
                if(passhash.length < index.length) {
                    var sha512sum = crypto.createHash('sha512');
                    sha512sum.update(passKey);
                    passhash = sha512sum.digest('hex');
                }

                for (var n = 0; n < index.length; n++) {
                    p.push(passhash.substr(n, 1));     
                } 
            ...
            if(toNum) {
                ...
                output = '';
                var start = math.floor( math.log(input, base));
                for (var i = start; i >= 0; i--) {
                    var bcp = math.pow(base, i);
